...
const possibilities_command = command_prefix + "possibilities";
...
client.on('message', msg => {
  ...
  // - - - - - - LIST_COMMAND - - - - 
    ...
    msg.reply(list);
  } else if (msg.content.startsWith(possibilities_command)) {



    msg.reply(output_possibilities);
  }
});
...
// Let's try predicting some prices...

//data = [94,75,72,67,109,124,148,163,153,57,52,47,43];
data = [NaN,75,72,67,109,124,148,163,undefined,undefined,undefined,undefined,undefined];
first_buy = false;
previous_pattern = undefined;
let dayNames = ['Buy', 'Mon AM', 'Mon PM', 'Tue AM', 'Tue PM', 'Wed AM', 'Wed PM', 'Thu AM', 'Thu PM', 'Fri AM', 'Fri PM', 'Sat AM', 'Sat PM'];


for (let poss of analyze_possibilities(data, first_buy, previous_pattern)) {
  var out_line = `**Possible pattern**: ${poss.pattern_description}\n`;
  out_line += `**Probability**: ${Number.isFinite(poss.probability) ? ((poss.probability * 100).toPrecision(3) + '%') : '—'}\n`;
  let dayArr = poss.prices.slice(1);
  for (let i = 0; i < dayArr.length; i++) {
    if (dayArr[i].min !== dayArr[i].max) {
      out_line += `${dayNames[i]}: ${dayArr[i].min} to ${dayArr[i].max} // `;
    } else {
      out_line += `${dayNames[i]}: ${dayArr[i].min} // `;
    }
  }
  out_line += `\nGuaranteed minimum: ${poss.weekGuaranteedMinimum} // Possible maximum: ${poss.weekMax}\n`;
  output_possibilities += out_line
}


console.log(output_possibilities);


// CODE FROM TURNIP PROPHET!!!!!!  - - -- - - -- - - - - - BASICALLY IGNORE IT. Taken on 2020-04-11. (https://github.com/mikebryant/ac-nh-turnip-prices/blob/master/js/predictions.js)
